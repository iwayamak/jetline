...
class TestCopyCommand(BaseTestCase):
    ...
    def test_copy_file(self):
        """ファイルコピーが成功することを確認する。."""
        source_file_path = os.path.join(TEST_DATA_PATH, "test_copy_command.csv")
        destination_file_path = os.path.join(TEST_DATA_PATH, "test_copy_command_dest.csv")

        if not os.path.exists(source_file_path):
            with open(source_file_path, "a", encoding="utf-8"):
                pass

        command = CopyCommand(source_file_path, destination_file_path)
        command.execute()
        self.assertTrue(os.path.exists(destination_file_path))

    def test_copy_directory(self):
        """ディレクトリコピーが成功することを確認する。."""
        source_directory_path = os.path.join(TEST_DATA_PATH, "test_copy_command_directory")
        ...
        command = CopyCommand(source_directory_path, destination_directory_path)
        command.execute()
